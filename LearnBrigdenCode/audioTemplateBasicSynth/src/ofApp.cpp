...
void ofApp::audioOut(ofSoundBuffer & buffer){
    ...
    //put your code below here-----------------------------------
        ...
        float carrierFreq = 440;
        float modRate = 1;
        float modIndex = 100;
        
        wave = myOsc.sinewave(carrierFreq + (modulator.sinewave(modRate)*modIndex));
        
        //This is our audio buffer and our speakers!
        audio[i] = buffer[i*buffer.getNumChannels()    ] = wave * 0.2;
        buffer[i*buffer.getNumChannels() + 1] = wave* 0.2;
